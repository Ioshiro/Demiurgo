ISImpassableHighMetalFence = ISBuildingObject:derive('ISImpassableHighMetalFence')

function ISImpassableHighMetalFence:create(x, y, z, north, sprite)
...
function ISImpassableHighMetalFence:addPart(x, y, z, north, sprite)
  local cell = getWorld():getCell()
  if not north then
  ...
  self.javaObject = IsoObject.new(cell, sq, sprite, north, self)

  sq:AddSpecialObject(self.javaObject)
  self.javaObject:transmitCompleteItemToServer()
end

function ISImpassableHighMetalFence:getHealth()
  return 200 + buildUtil.getWoodHealth(self)
end

function ISImpassableHighMetalFence:render(x, y, z, square)
...
function ISImpassableHighMetalFence:isValid(square)
...
function ISImpassableHighMetalFence:getSquare2Pos(square, north)
  local x = square:getX()
  local y = square:getY()
  local z = square:getZ()
  if north then
...
function ISImpassableHighMetalFence:new(sprite1, sprite2, northSprite1, northSprite2)
  local o = {}
  setmetatable(o, self)
  self.__index = self
  o:init()
